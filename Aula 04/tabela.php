...
$arrayFiltrado = array_filter($pessoas, function ($pessoa) {
  ...
  $idadePersona = calculaIdade($pessoa['dtNascimento']);
  if( (empty($nome) || (stripos($pessoa["nome"], $nome) > -1)) && (empty($idade) || $idade == $idadePersona) && (empty($cidade) || (stripos($pessoa["cidade"], $cidade) > -1)) && (empty($estado) || (stripos($pessoa["cidade"], $cidade) > -1)) && (empty($estado) || $pessoa["estado"] == $estado) ) {
    return true;
  }
});

function GerarTabela ($arrayFiltrado) {
  echo "<table border='1px'>"; 
    ...
    foreach($arrayFiltrado as $pessoa) {
      echo "<tr>";
